...
export function DynamicGroupsTable({
  ...
  const [viewMembersGroupId, setViewMembersGroupId] = useState<string | null>(
    null,
  );
  ...
  const handleDelete = async () => {
    if (deleteGroupId) {
      try {
        setIsDeleting(true);
        await deleteGroup({
          id: deleteGroupId as Id<"dynamic_computer_groups">,
        });
        ...
        <div className="flex flex-col items-center gap-1 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Define rules that automatically assign computers to groups based
              on their properties.
            </p>
          </div>
