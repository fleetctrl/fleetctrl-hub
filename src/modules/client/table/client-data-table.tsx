...
export function ClientUpdatesTable() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const uploadedFile = await uploadFileToConvex(
        uploadFile,
        generateUploadUrl,
      );
      ...
      <div className="flex flex-col items-center gap-1 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground flex flex-col">
            <span>
              Manage client versions for automatic updates on enrolled
              computers.
            </span>
            {activeVersion && (
          ...
          <Dialog open={uploadDialogOpen} onOpenChange={setUploadDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Upload a new FleetCtrl client binary. The hash will be
                  calculated automatically for integrity verification.
                </DialogDescription>
              </DialogHeader>
