  ...
  try {
    ...
    if (clientVersion) {
      try {
        ...
        const activeVersion = await c.env.ctx.runQuery(
          internal.client.getActiveVersion,
          {},
        );
...
protectedApi.get("/tasks", async (c) => {
  ...
  const tasks = await c.env.ctx.runQuery(internal.tasks.getPending, {
    computerId,
  });
  return c.json({ tasks });
});
...
protectedApi.get("/apps/assigned", async (c) => {
  const computerId = c.var.computerId;
  const apps = await c.env.ctx.runQuery(internal.apps.getAssigned, {
    computerId,
  });
  return c.json({ apps });
});
...
protectedApi.get("/client/download/:versionId", async (c) => {
  ...
  const downloadUrl = await c.env.ctx.runAction(
    internal.client.getDownloadUrl,
    {
      versionId,
    },
  );
...
app.get("/client/version", async (c) => {
  const version = await c.env.ctx.runQuery(
    internal.client.getActiveVersion,
    {},
  );
