...
export const enroll = action({
    ...
    handler: async (ctx, { enrollmentToken, name, fingerprintHash, jkt }) => {
        ...
        const existing = await ctx.runQuery(
            // @ts-expect-error - internal API
            "auth:getComputerByFingerprintHash",
            { fingerprintHash }
        );
