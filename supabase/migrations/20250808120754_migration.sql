alter table "public"."tasks" add column "error" character varying;

alter table "public"."tasks" add column "task_data" json;

alter table "public"."tasks" alter column "status" set default 'PENDING'::"task status";
...
CREATE OR REPLACE FUNCTION public.get_tasks_by_rustdesk_id(in_rustdesk_id integer, in_key uuid)
 ...
 SET search_path TO ''
AS $function$SELECT public.tasks.created_at, 
  public.tasks.status, 
  public.tasks.task,
...
create policy "Enable delete to authenticated"
on "public"."computers"
as permissive
for delete
to public
